...
export const statusSmsReportsAPI = {
  list: (from?: string, to?: string) =>
    fetchProtected(
      `/admin/reports/sms/by-status/?${from ? `from=${from}&` : ""}${
        to ? `to=${to}` : ""
      }`
    ),
  getById: (id: string) =>
    fetchProtected(`/admin/reports/sms/by-status/${id}/`),
  create: (data: Partial<StatusSmsReport>) =>
    fetchProtected(`/admin/reports/sms/by-status/`, {
      method: "POST",
      body: JSON.stringify(data),
    }),
  update: (id: string, data: Partial<StatusSmsReport>) =>
    fetchProtected(`/admin/reports/sms/by-status/${id}/`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
